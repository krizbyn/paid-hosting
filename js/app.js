(() => {
    ...
    window.onload = function() {
        ...
        function documentActions(e) {
            ...
            if (window.innerWidth < 768) if (targetElement.classList.contains("menu__arrow")) targetElement.closest(".menu__item").classList.toggle("_hover");
        }
        ...
        if (isIE()) document.querySelector("html").classList.add("ie");
        if (isMobile.any()) document.querySelector("html").classList.add("_touch");
        function testWebP(callback) {
            var webP = new Image;
            webP.onload = webP.onerror = function() {
                callback(2 == webP.height);
        ...
        if (location.hash) {
            ...
            if (document.querySelector(".popup_" + hsh)) popup_open(hsh); else if (document.querySelector("div." + hsh)) _goto(document.querySelector("." + hsh), 500, "");
        }
        ...
        if (title) for (let index = 0; index < title.length; index++) {
            let el = title[index];
        ...
        for (let index = 0; index < popups.length; index++) {
            ...
            popup.addEventListener("click", (function(e) {
                if (!e.target.closest(".popup__body")) popup_close(e.target.closest(".popup"));
            }));
        }
        function popup_open(item, video = "") {
            let activePopup = document.querySelectorAll(".popup._active");
            if (activePopup.length > 0) popup_close("", false);
        ...
        function popup_close(item, bodyUnlock = true) {
            if (unlock) {
                if (!item) for (let index = 0; index < popups.length; index++) {
                    const popup = popups[index];
                ...
                if (!document.querySelector(".menu__body._active") && bodyUnlock) body_lock_remove(500);
        ...
        if (moreBlocks.length > 0) {
            ...
            for (let index = 0; index < moreBlocks.length; index++) {
                ...
                if (items.length > 0) {
                    ...
                    if ("0s" === getComputedStyle(itemsContent).getPropertyValue("transition-duration")) itemsContent.style.cssText = "transition-duration: 1ms";
                    itemsMore.addEventListener("click", (function(e) {
                        if (itemsMore.classList.contains("_active")) setSize(); else setSize("start");
                    ...
                    function setSize(type) {
                        ...
                        for (let index = 0; index < items.length; index++) {
                            if (index < itemsView) itemsContentHeight += items[index].offsetHeight;
                            itemsContentStartHeight += items[index].offsetHeight;
                        }
                        resultHeight = "start" === type ? itemsContentStartHeight : itemsContentHeight;
                    ...
                    function updateSize() {
                        ...
                        if (0 === isScrollStart && isScrollEnd > 0 || isScrollStart > 0 && 0 === isScrollEnd) if (itemsMore.classList.contains("_active")) setSize("start"); else setSize();
                    }
